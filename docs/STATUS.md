...
- iOS: after the TestFlight feedback changes, local Duo UI passed 4/4 and Swift core 26/26. The Remove-alert fix moves Remove into the parent alert and is in open PR #2 (`50fec6b`); checks are pending. Manual Device Hub testing confirmed selected-person retention across close/open. Duo is currently booted and iPhone 17 Pro is shut down.
...
- PR #1 merged to `main` at `8ebdc0f`; API, iOS, and web checks passed on the PR. Its post-merge iOS UI failure is addressed by open PR #2 (`50fec6b`), which moves Remove into the parent alert; PR checks are pending. Render deployment `dep-dar0hsivcj2c739t5hfg` is live for the merge commit. `/health/ready` returned `200 Healthy`; an unauthenticated avatar PUT returned `401`. The new API preset allowlist changes are not deployed.
...
- **Duo/release validation:** local UI suite passed 4/4 after the feedback work and independent review found no issue. The closed-Duo screen was reviewed; far-edge carousel and open-Duo visual sign-off remain. The Remove-alert fix is in open PR #2 (`50fec6b`); checks are pending.
