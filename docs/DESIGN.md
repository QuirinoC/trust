...
- **You:** focus on profile picture, membership, My location, and account. Keep Terms, Privacy, and Support links fixed at the bottom of the screen. My location opens a detail screen for device permission and Home place, while the sharing status controls live in Sharing.
- **Profile pictures:** offer 26 illustrated choices in a horizontal carousel, with no visible names; expose each choice's name to accessibility. Keep the selected picture large and centered in the preview. Include Camera and Photos as alternatives. Stage changes until Save. A picture is visible only to active connected people; removing it falls back to initials. Preserve initials for missing or failed photo loads. Do not expose raw photo URLs outside authenticated API calls. New icon art should use one centered, softly sculpted subject with flowing contours, a seamless color-wash background, and no text or border; keep it legible inside a small circular crop.
